@extends('layouts.app')
<style>
    /* Sembunyikan menu dengan teks tertentu */
    nav a[href*="home"],
    nav a[href*="personal-achievement"],
    nav a[href*="leaderboard"],
    nav a[href*="promo"],
    nav a[href*="profile"],
    nav #mobile-menu a[href*="home"],
    nav #mobile-menu a[href*="personal-achievement"],
    nav #mobile-menu a[href*="leaderboard"],
    nav #mobile-menu a[href*="promo"],
    nav #mobile-menu a[href*="profile"],
            ...
            <div id="productGrid" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                @foreach ($barangs as $produk)
                    <div class="product-card bg-white border border-black rounded-lg shadow p-3 flex flex-col items-center text-center"
                        ...
                        <img src="{{ $produk->gambar }}" class="w-full h-32 object-contain mb-3" alt="{{ $produk->NAMAB }}">
...
@section('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const logoutBtn2 = document.getElementById('button-logout2');
            if (logoutBtn && logoutBtn2) {
                logoutBtn.innerText = 'Login';
                logoutBtn2.innerText = 'Login';
            }
        });
    </script>
@endsection
